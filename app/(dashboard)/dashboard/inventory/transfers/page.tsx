...
const STATUSES: TransferStatus[] = ["Pending", "InTransit", "Received", "Cancelled"];
...
export default function Transfers() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success(`${t.transferNumber} তৈরি হয়েছে`);
      setOpen(false);
      reset();
    } catch (err) {
  ...
  const statusBadge = (s: TransferStatus) => {
    const cls =
      ...
      s === "InTransit" ? "border-primary text-primary" :
          ...
          {detail && (
            <div className="mt-4 space-y-4 text-sm">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {(detail.status === "Pending" || detail.status === "InTransit") && (
                  <>
                    <Button
