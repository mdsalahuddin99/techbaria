...
import { useProducts, useDeleteProduct, useUpdateProduct, useBulkUpdateProducts, useBulkDeleteProducts } from "@/features/products/hooks";
...
// Categories are sourced from the store-managed category list so the UI
// stays clean until the shop owner creates their own categories.

// BulkState moved to features/products/components/BulkEditDialog
...
export default function Products() {
  ...
  const handleScanned = (code: string) => {
    ...
    if (!code) return;
    if (scanTarget === "form") {
      // Dialog is already open — set the prefill and the form's effect picks it up.
                  ...
                  {filtered.map((p) => {
                    ...
                    return (
                      <TableRow key={p.id} data-state={isSelected ? "selected" : undefined}>
                        ...
                        <TableCell className="text-right">
                          <Button size="icon" variant="ghost" onClick={() => setViewProduct(p)} aria-label={`View ${productDisplayName(p)}`} title="View details">
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => setLabelProducts([p])} aria-label={`Print label for ${productDisplayName(p)}`} title="Print label">
                            <Tag className="h-4 w-4" />
                          </Button>
                          <Button size="icon" variant="ghost" onClick={() => openEdit(p)} aria-label={`Edit ${productDisplayName(p)}`}>
                            <Pencil className="h-4 w-4" />
                          </Button>
                          <Button size="icon" variant="ghost" className="text-destructive" onClick={() => setDeleteId(p.id)} aria-label={`Delete ${productDisplayName(p)}`}>
