...
export default function Customers() {
  ...
  const filtered = useMemo(() => {
    ...
    const base = customers.filter((c) => {
      const d = Math.max(0, c.due ?? 0);
      if (dueFilter === "with-due" && d <= 0) return false;
      if (dueFilter === "no-due" && d > 0) return false;
      return (
    ...
    return sorted;
  }, [customers, search, dueFilter, sortKey, sortDir]);
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Select value={dueFilter} onValueChange={(v) => setDueFilter(v as typeof dueFilter)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="with-due">With due ({dueCount})</SelectItem>
              <SelectItem value="no-due">No due ({customers.length - dueCount})</SelectItem>
            </SelectContent>
          </Select>
          <Select
