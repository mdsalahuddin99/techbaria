...
export default function StorefrontCompare() {
  ...
  const rows: { label: string; get: (p: typeof items[number]) => React.ReactNode }[] = [
    { label: "Price", get: (p) => <span className="text-[#16A34A] font-bold">{formatPrice(p.price)}</span> },
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-10">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight flex items-center gap-2 text-[#1E3A5F]">
          <GitCompareArrows className="h-6 w-6 text-[#16A34A]" /> Compare
        </h1>
      ...
      <div className="overflow-x-auto rounded-2xl border border-[#E2E8F0] bg-white shadow-sm">
        <table className="w-full text-sm min-w-[640px]">
          <thead>
            <tr>
              <th className="sticky left-0 bg-[#F0FDF4] z-10 p-4 text-left text-xs text-slate-500 font-medium uppercase tracking-wider w-32">
                Feature
              </th>
              {items.map((p) => (
                <th key={p.id} className="p-4 text-left align-top min-w-[180px]">
                  <div className="relative rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3">
                    ...
                    <Link href={`/storefront/p/${encodeURIComponent(p.id)}`} className="text-sm font-semibold text-[#1E3A5F] hover:text-[#16A34A] line-clamp-2">
                      {p.name}
                    </Link>
                  </div>
                </th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} className="border-t border-[#E2E8F0]">
                <td className="sticky left-0 bg-[#F0FDF4] z-10 p-3 text-xs text-slate-500 font-medium">{row.label}</td>
                {items.map((p) => (
