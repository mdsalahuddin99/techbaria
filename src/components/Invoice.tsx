...
function numberToWords(num: number): string {
  ...
  const under1000 = (x: number): string => {
    ...
    if (x >= 20) { s += b[Math.floor(x / 10)]; if (x % 10) s += " " + a[x % 10]; }
    else if (x > 0) { s += a[x]; }
    return s;
  };
  const parts: string[] = [];
    ...
    <tr>
      <td style="width:62%;vertical-align:top;border-right:1px solid #888;padding:7px 10px;">
        <table style="border-collapse:collapse;width:100%;">
          <tr><td style="font-weight:700;width:88px;padding:2.5px 0;vertical-align:top;">Customer</td><td style="padding:2.5px 0;font-weight:700;vertical-align:top;">: ${esc(sale.customerName || "—")}</td></tr>
          <tr><td style="font-weight:600;padding:2px 0;vertical-align:top;">Address</td><td style="padding:2px 0;vertical-align:top;">: ${esc(sale.customerAddress || "")}</td></tr>
          <tr><td style="font-weight:600;padding:2px 0;vertical-align:top;">Mobile</td><td style="padding:2px 0;vertical-align:top;">: ${esc(sale.customerPhone || "")}</td></tr>
          <tr><td style="font-weight:600;padding:2px 0;vertical-align:top;">Attention</td><td style="padding:2px 0;vertical-align:top;">: ${esc(sale.attention || sale.customerReferencePerson || "")}</td></tr>
          <tr><td style="font-weight:600;padding:2px 0;vertical-align:top;">Destination</td><td style="padding:2px 0;vertical-align:top;">: ${esc(sale.destination || "")}</td></tr>
        </table>
      </td>
      <td style="width:38%;vertical-align:top;padding:0;">
        <table style="border-collapse:collapse;width:100%;height:100%;">
          <tr style="border-bottom:1px solid #888;"><td style="padding:4px 8px;font-weight:700;width:102px;background:#f5f5f5;border-right:1px solid #888;">Invoice No.</td><td style="padding:4px 8px;font-weight:700;">${esc(sale.invoiceNo)}</td></tr>
          ...
          <tr style="border-bottom:1px solid #888;"><td style="padding:4px 8px;font-weight:700;background:#f5f5f5;border-right:1px solid #888;">Prepared By</td><td style="padding:4px 8px;">${esc(sale.cashier || "—")}</td></tr>
          <tr style="border-bottom:1px solid #888;"><td style="padding:4px 8px;font-weight:700;background:#f5f5f5;border-right:1px solid #888;">Sales Person</td><td style="padding:4px 8px;">${esc(sale.salesPerson || "—")}</td></tr>
          <tr><td style="padding:4px 8px;font-weight:700;background:#f5f5f5;border-right:1px solid #888;">Bill Status</td><td style="padding:4px 8px;font-weight:700;color:${billStatusColor};">${billStatus}</td></tr>
        </table>
      </td>
    </tr>
  </table>
  ...
  <table style="border:1px solid #888;font-size:10.5px;margin-bottom:13px;">
    <tr>
      <td style="width:54%;vertical-align:top;border-right:1px solid #888;padding:8px 10px;">
        ...
        <div style="margin-bottom:10px;"><span style="font-weight:700;">Narration : </span><span>${esc(sale.notes || "")}</span></div>
        <div style="font-weight:700;margin-bottom:4px;">Return &amp; Refund :</div>
      ...
      <td style="width:46%;vertical-align:top;padding:0;">
        <table style="border-collapse:collapse;width:100%;font-size:10.5px;">
          <tr style="border-bottom:1px solid #ccc;"><td style="padding:5px 10px;font-weight:600;background:#f5f5f5;border-right:1px solid #ccc;">Total Amount</td><td style="padding:5px 10px;text-align:right;font-weight:600;">${sale.subtotal.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1px solid #ccc;"><td style="padding:5px 10px;font-weight:600;background:#f5f5f5;border-right:1px solid #ccc;">Less Discount</td><td style="padding:5px 10px;text-align:right;">${sale.discount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1px solid #ccc;"><td style="padding:5px 10px;font-weight:600;background:#f5f5f5;border-right:1px solid #ccc;">Add VAT</td><td style="padding:5px 10px;text-align:right;">${(sale.vat ?? 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1px solid #ccc;"><td style="padding:5px 10px;font-weight:600;background:#f5f5f5;border-right:1px solid #ccc;">Add Extra Charges</td><td style="padding:5px 10px;text-align:right;">${(sale.extraCharges ?? 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1.5px solid #666;background:#efefef;"><td style="padding:6px 10px;font-weight:700;font-size:11px;border-right:1px solid #ccc;">Net Payable Amount</td><td style="padding:6px 10px;text-align:right;font-weight:700;font-size:11px;">${sale.total.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1px solid #ccc;"><td style="padding:5px 10px;font-weight:600;background:#f5f5f5;border-right:1px solid #ccc;">Previous Due</td><td style="padding:5px 10px;text-align:right;">${previousDue.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1.5px solid #666;background:#efefef;"><td style="padding:6px 10px;font-weight:700;font-size:11px;border-right:1px solid #ccc;">Total Due Amount</td><td style="padding:6px 10px;text-align:right;font-weight:700;font-size:11px;">${totalDueAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="border-bottom:1px solid #ccc;"><td style="padding:5px 10px;font-weight:600;background:#f5f5f5;border-right:1px solid #ccc;">Received Amount</td><td style="padding:5px 10px;text-align:right;font-weight:700;">${receivedShown.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
          <tr style="background:${currentDueAmount > 0 ? "#fff3f3" : "#f0faf4"};"><td style="padding:6px 10px;font-weight:700;font-size:11px;border-right:1px solid #ccc;">Current Due Amount</td><td style="padding:6px 10px;text-align:right;font-weight:700;font-size:11px;color:${currentDueAmount > 0 ? "#dc2626" : "#16a34a"};">${currentDueAmount.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td></tr>
        </table>
      </td>
    </tr>
  </table>

  <!-- WARRANTY VOID NOTICE -->
  <div style="border:1.5px solid #555;text-align:center;padding:6px 16px;font-size:10.5px;margin-bottom:26px;font-weight:600;color:#222;background:#fffde7;">
    <strong>Warranty Void</strong> - ${esc(settings.invoiceWarrantyNote || "The Warranty Is Not Applicable To Adaptor, Remote Control, Sticker-removed Items, Burnt & Physically Damaged Item.")}
  </div>
