...
export const ledgerService = {
  ...
  async listLedger(ctx: Ctx): Promise<LedgerTransaction[]> {
    ...
    // Customer wallet transactions (PAYMENT = in, REFUND = out)
    for (const ct of customerWalletTxs) {
      const isPayment = ct.type === "PAYMENT";
      txs.push({
        id: ct.id,
        accountId: ct.accountId!,
        date: ct.createdAt.toISOString(),
        direction: isPayment ? "in" : "out",
        amount: Number(ct.amount),
        category: isPayment ? "due_collection" : "withdraw",
        refType: "manual",
        refId: ct.id,
        note:
          ct.notes ||
          `Customer wallet ${isPayment ? "deposit" : "withdraw"} — ${ct.customer?.name ?? "unknown"}`,
        createdBy: "System",
      });
    }
