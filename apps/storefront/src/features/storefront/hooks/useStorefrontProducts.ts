...
export interface StorefrontFilters {
  ...
  sort?: SortKey;
  inStockOnly?: boolean;
  /** Only products with a compareAtPrice > price. */
  onSaleOnly?: boolean;
...
export function deriveFlashDeals(products: StorefrontProduct[], limit = 6): StorefrontProduct[] {
  ...
  return (flashDeals.length ? flashDeals : fallback).slice(0, limit);
}

/** Pure function to derive trending products. */
export function deriveTrendingProducts(products: StorefrontProduct[], limit = 6): StorefrontProduct[] {
  return products.filter((p) => p.isTrending).slice(0, limit);
}

/** "Flash deals" — for v1 we surface products with a compareAtPrice or low stock. */
