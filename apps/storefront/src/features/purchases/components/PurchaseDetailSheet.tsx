...
export function PurchaseDetailSheet({
  ...
  const { addPayment, receive: receivePurchase } = usePurchaseActions();
            ...
            <Card className="p-3 bg-secondary/40 space-y-3">
                ...
                <div>
                  ...
                  {accounts.length === 0 ? (
                    <div className="text-xs text-muted-foreground border rounded-md p-2">
                      Accounts page থেকে account add করুন
                    </div>
                  ) : (
              ...
              {payAccount && (
                <p className="text-[11px] text-muted-foreground">
                  Method: <strong>{methodFromAccountType(payAccount.type)}</strong> · বর্তমান balance {formatCurrency(balances[payAccount.id] ?? 0)}
                </p>
              )}
              <div>
