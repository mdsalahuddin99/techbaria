...
function serialiseStorefrontOne(p: any): StorefrontProduct {
  ...
  let defaultDiscount: { mode: "percent" | "flat"; value: number } | undefined = undefined;
  ...
  const priceToUse = p.onlinePrice != null ? Number(p.onlinePrice) : Number(p.price);
  let compareAtPrice: number | undefined = p.compareAtPrice != null ? Number(p.compareAtPrice) : undefined;
  // Fallback for defaultDiscount if compareAtPrice is not set
  if (!compareAtPrice && defaultDiscount) {
     if (defaultDiscount.mode === "percent") {
         compareAtPrice = priceToUse / (1 - (defaultDiscount.value / 100));
     } else {
         compareAtPrice = priceToUse + defaultDiscount.value;
     }
  }
