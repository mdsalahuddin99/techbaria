...
export function SuppliersClient({
  ...
  const initialInfiniteData = useMemo(() => {
    const items = initialSuppliers.slice(0, 15);
      ...
      <Card className="p-3">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 justify-between items-start lg:items-center">
          ...
          <div className="flex flex-wrap gap-2 items-center w-full lg:flex-1 lg:justify-end">
            ...
            {isFilterEmpty && suppliers.length >= 15 && (
        ...
        <SheetContent className="sm:max-w-xl overflow-y-auto">
          ...
          {detail && (
            <Tabs defaultValue="overview" className="mt-4 w-full">
              ...
              <TabsContent value="overview" className="space-y-4">
                <Card className="p-4 bg-secondary/20">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Info label="Email" value={detail.email || "—"} className="col-span-1 sm:col-span-2" />
                    <Info label="Joined Date" value={formatDate(detail.createdAt)} />
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Card className={`p-3 border-l-4 ${(detail.advanceBalance || 0) > 0 ? "border-l-green-500 bg-green-500/5 dark:bg-green-500/10" : "border-l-muted bg-secondary/10"}`}>
                    ...
                    <p className={`text-xl font-bold ${(detail.advanceBalance || 0) > 0 ? "text-green-600 dark:text-green-400" : ""}`}>{formatCurrency(detail.advanceBalance || 0)}</p>
                  </Card>
                </div>
              </TabsContent>
                ...
                <div className="border rounded-md overflow-hidden bg-background">
                  <Table>
                    <TableHeader className="bg-muted/30">
                      <TableRow>
                        <TableHead className="h-8 py-1">PO #</TableHead>
              ...
              <TabsContent value="ledger" className="m-0 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium">Wallet & Ledger History</p>
                  <Badge variant="outline" className="font-mono bg-green-500/5 text-green-600 dark:text-green-400 border-green-500/20">{formatCurrency(detail.advanceBalance || 0)} Avail.</Badge>
                </div>
