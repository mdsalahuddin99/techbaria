...
import { Plus, Search, Pencil, Trash2, Settings2, X, ScanLine, PackageOpen, MoreHorizontal, Tag } from "lucide-react";
...
export function ProductsClient({
  ...
  const [quickPriceEditing, setQuickPriceEditing] = useState<Product | null>(null);
  const [quickPriceForm, setQuickPriceForm] = useState<{ onlinePrice?: number; compareAtPrice?: number; isFlashDeal?: boolean }>({});

  const handleSaveQuickPrice = async () => {
    if (!quickPriceEditing) return;
    try {
      await updateMutation.mutateAsync({
        id: quickPriceEditing.id,
        patch: {
          onlinePrice: quickPriceForm.onlinePrice,
          compareAtPrice: quickPriceForm.compareAtPrice,
          isFlashDeal: quickPriceForm.isFlashDeal,
        }
      });
      setQuickPriceEditing(null);
      toast.success("Online prices updated");
    } catch (e: any) {
    ...
    <div className="space-y-4">
      ...
      <Card className="p-3">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {isFilterEmpty && products.length >= 15 && (
                        ...
                        <div className="flex items-center gap-1 ml-auto">
                          {filterOnlineOnly && (
                            <Button size="icon" variant="ghost" className="h-7 w-7 text-primary" onClick={() => {
                              setQuickPriceEditing(p);
                              setQuickPriceForm({
                                onlinePrice: (p as any).onlinePrice,
                                compareAtPrice: (p as any).compareAtPrice,
                                isFlashDeal: (p as any).isFlashDeal,
                              });
                            }}>
                            ...
                            <DropdownMenuContent align="end">
                              {filterOnlineOnly && (
                                <DropdownMenuItem onClick={() => {
                                  ...
                                  <Tag className="mr-2 h-4 w-4" /> Quick Price Edit
                                </DropdownMenuItem>
                              )}
      ...
      <Dialog open={!!quickPriceEditing} onOpenChange={(o) => !o && setQuickPriceEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Quick Edit Online Prices</DialogTitle>
            <DialogDescription>
              Set special prices for the storefront. Leave blank to use POS prices automatically.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input 
                type="number" 
                value={quickPriceForm.onlinePrice ?? ""} 
                onChange={(e) => setQuickPriceForm({ ...quickPriceForm, onlinePrice: e.target.value ? Number(e.target.value) : undefined })}
                placeholder="e.g. 900"
              />
            </div>
            <div className="space-y-2">
              <Label>Compare At Price (Crossed out)</Label>
              <Input 
                type="number" 
                value={quickPriceForm.compareAtPrice ?? ""} 
                onChange={(e) => setQuickPriceForm({ ...quickPriceForm, compareAtPrice: e.target.value ? Number(e.target.value) : undefined })}
                placeholder="e.g. 1200"
              />
            </div>
            <div className="flex items-center justify-between mt-4">
              <Label htmlFor="flash-deal-switch" className="cursor-pointer">Mark as Flash Deal</Label>
              <Switch 
                id="flash-deal-switch"
                checked={quickPriceForm.isFlashDeal ?? false}
                onCheckedChange={(c) => setQuickPriceForm({ ...quickPriceForm, isFlashDeal: !!c })}
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setQuickPriceEditing(null)}>Cancel</Button>
            <Button onClick={handleSaveQuickPrice}>Save Prices</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
