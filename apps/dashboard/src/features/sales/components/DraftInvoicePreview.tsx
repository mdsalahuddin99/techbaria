...
function buildDraftInvoiceHtml(
  ...
  const grossSubtotal = round2(
    cartItems.reduce((s: number, r: any) => s + r.price * r.qty, 0)
  );
  const totalDiscount = round2(
    cartItems.reduce((s: number, r: any) => s + (r.discount || 0), 0) + (draft.discount || 0)
  );
  const subtotal = grossSubtotal;
  const total = round2(Math.max(0, grossSubtotal - totalDiscount));
