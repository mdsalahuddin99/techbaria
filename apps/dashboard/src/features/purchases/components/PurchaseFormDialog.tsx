                  ...
                  return (
                    <Card key={l.productId} className={isCollapsed ? "p-2.5" : "p-3 space-y-3 border-primary"}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                        <div>
                          <label className="text-xs text-muted-foreground">{l.isBundle ? "Total Cost ৳ *" : "Cost ৳ *"}</label>
                          {l.isBundle ? (
                            <Input type="number" value={l.totalCost === 0 ? "" : l.totalCost} onChange={(e) => form.updateLine(l.productId, { totalCost: Number(e.target.value) || 0 })} />
                          ) : (
                ...
                <div className="space-y-2">
                  {form.tenders.map((t, idx) => (
                    <div key={t.id} className="grid grid-cols-1 sm:grid-cols-[120px_1fr_140px_auto] gap-2 items-start">
                      ...
                      {t.method === "WALLET" ? (
                        <div className="flex items-center px-3 h-10 border rounded-md bg-green-50/50 dark:bg-green-900/20 text-sm font-semibold text-green-700 dark:text-green-400">
