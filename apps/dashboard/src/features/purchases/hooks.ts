...
export function useDeletePurchasePayment() {
  ...
  return useMutation({
    mutationFn: ({ purchaseId, paymentId }: { purchaseId: string; paymentId: string }) =>
      purchasesService.removePayment(purchaseId, paymentId),
    onSuccess: () => invalidateProcurement(qc),
    onError: (e: Error) => toast.error(e.message),
  });
}
...
export function usePurchaseActions() {
  ...
  const delPayM = useDeletePurchasePayment();
  ...
  return {
    ...
    deletePayment: (purchaseId: string, paymentId: string) => delPayM.mutateAsync({ purchaseId, paymentId }),
